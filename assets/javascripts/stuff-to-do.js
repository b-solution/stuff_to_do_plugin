...
jQuery(function($) {
  ...
  attachSortables();
    ...
    $("#notification").click(function(e) {
        comment = $('#comment_notification').children('#comment').val() || '';
        $("div.notice").hide()
        $("div.error").hide()
        $.ajax({
            type: "GET",
            url: '/stuff_to_do/notification.js',
            dataType: 'json',
            data: $("#user_id").serialize()+"&comment="+comment,
            success: function(response) {
                if(response['success'])
                {
                    $("div.notice").html(response['msg']).show();
                }
                else
                {
                    $("div.error").html(response['msg']).show();
                }
            ...
            error: function(response) {
                $("div.error").html("Error Sending notification.  Please refresh the page.").show();
            }});

    });
    $('#comment_notification').children('form').submit(function(e){
        e.preventDefault();
