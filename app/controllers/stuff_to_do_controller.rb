  ...
  def notification
    respond_to do |format|
      ...
      format.js {
        ...
        stuff_to_dos = StuffToDo.where(user_id: user.id)
        if stuff_to_dos.present?
          comment = params[:comment] || ''
          Mailer.recommended_below_threshold(user, stuff_to_dos, comment).deliver
          render :json => {success: true, msg: "Mail sent to #{user}."}
        else
          render :json => {success: false, msg: "This user don't have any todo list. please add some todo task to him" }
        end
